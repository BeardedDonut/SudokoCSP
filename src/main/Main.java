                ...
                while ((line = in.readLine()) != null) {
                    for (int run = 0; run < runs; run++) {
                        if (run == 0) {
                            ...
                            sudo = new Sudoku(size);
                            int cell_id, cell_num;
                            String row;
                            // reading sudoku board
                            ...
                            for (int x = 0; x < sudo.getBoardSize(); x++) {
                                row = in.readLine();
                                for (int y = 0; y < sudo.getBoardSize(); y++) {
                                    int correspondingCellId = x * 9 + y;
                                    int regId = Character.getNumericValue(row.charAt(y));

                                    RegionMap myReg = sudo.getRegion(regId);
                                    Cell cell = sudo.getCells().get(correspondingCellId);
                                    myReg.addCell(cell);
                                }
                            }
